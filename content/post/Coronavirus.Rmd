---
title: "Coronavirus"
author: "Megha Joshi"
date: 2020-03-27
categories: ["R"]
tags: ["coronavirus", "COVID-19", "ggplot", "leaflet"]

---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

In this post, I am analyzing data on COVID-19 cases worldwide. 

# Libraries

```{r message = FALSE, warning = FALSE}
library(tidyverse)
library(maps)
library(ggthemes)
library(jsonlite)
library(leaflet)
library(widgetframe)
library(lubridate)
library(scales)
library(ggrepel)
```


# Read in the Data

I downloaded the data from [here](https://github.com/CSSEGISandData/COVID-19).

```{r, warning = FALSE, message = FALSE}
confirmed <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv") %>%
  pivot_longer(cols = -c(1:4),
               names_to = "date",
               values_to = "confirmed")

deaths <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv") %>%
  pivot_longer(cols = -c(1:4),
               names_to = "date",
               values_to = "deaths")

recovered <- read_csv("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv") %>% pivot_longer(cols = -c(1:4),
               names_to = "date",
               values_to = "recoveries")

# Join and Clean 
dat <- left_join(confirmed, deaths) %>%
  left_join(recovered) %>%
  mutate(date = mdy(date),
         lab = paste0("Confirmed: ", confirmed, ", ", "Deaths = ", deaths, ", ", "Recovered: ", recoveries)) %>%
  rename(country_region = `Country/Region`)
```


# Coronavirus Confirmed Cases

```{r}
covid_leaf <- leaflet(data = dat, options = leafletOptions(minZoom = 1.5)) %>% 
  addProviderTiles(providers$CartoDB.DarkMatter)  %>%
  addCircleMarkers(~Long, ~Lat, radius = ~confirmed/1000, label = ~lab, fillOpacity = .2)

frameWidget(covid_leaf)
```


# Cases by Week

## Confirmed 

```{r, fig.width=9, fig.height=6}
country_dat <- dat %>%
  filter(country_region %in% c("China", "Italy", "Iran", "Korea, South", "Spain", "US")) %>%
  mutate(week = cut(date, "week", start.on.monday = FALSE))

country_dat %>%
  ggplot(aes(x = confirmed, y = week, fill = country_region)) +
  geom_bar(stat = "identity", position = "dodge") + 
  facet_wrap(~ country_region) +
  labs(y = "", x = "Confirmed Cases") + 
  scale_x_continuous(labels = comma) +
  scale_fill_brewer(type = "qual", palette = 2) + 
  theme_bw() +
  theme(legend.position = "none")
```

## Deaths

```{r, fig.width=9, fig.height=6}
country_dat %>%
  ggplot(aes(x = deaths, y = week, fill = country_region)) +
  geom_bar(stat = "identity", position = "dodge") + 
  facet_wrap(~ country_region) +
  labs(y = "", x = "Deaths") + 
  scale_x_continuous(labels = comma) +
  scale_fill_brewer(type = "qual", palette = 2) + 
  theme_bw() +
  theme(legend.position = "none")
```

## Recoveries

```{r, fig.width=9, fig.height=6}
country_dat %>%
  ggplot(aes(x = recoveries, y = week, fill = country_region)) +
  geom_bar(stat = "identity", position = "dodge") + 
  facet_wrap(~ country_region) +
  labs(y = "", x = "Recovered") + 
  scale_x_continuous(labels = comma) +
  scale_fill_brewer(type = "qual", palette = 2) + 
  theme_bw() +
  theme(legend.position = "none")
```

# Confirmed Cases Trend

```{r}
sum_dat <- country_dat %>%
  group_by(date, country_region) %>%
  summarize(confirmed = sum(confirmed)) %>%
  ungroup() %>%
  mutate(country_region = if_else(country_region == "Korea, South", "S. Korea", country_region))
  
sum_dat %>%  
  ggplot(aes(x = date, y = confirmed, color = country_region)) +
  geom_line(size = 1) +
  geom_text(data = sum_dat %>% filter(date == last(date)), 
            aes(label = country_region, x = date + .5, 
                y = confirmed, color = country_region), hjust = -.001) +
  scale_y_continuous(labels = comma) +
  xlim(ymd("2020-01-22"), ymd("2020-04-01")) + 
  scale_color_brewer(type = "qual", palette = 2) + 
  labs(x = "", y = "Confirmed Cases", color = "Country") + 
  ggtitle("Confirmed Cases as of March 27, 2020") +
  theme_bw() +
  theme(legend.position = "none")
  
```

