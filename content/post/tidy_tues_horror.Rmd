...
```{r}
dat <- dat %>%
  mutate(genres_rec = str_remove_all(genres, "\\|"))  # remove |

dat_long <- dat %>% 
  separate_rows(genres_rec, sep = " ")  # long format

# Just to check - looks okay - just 1 movie with no genre
table(dat_long$genres_rec, useNA = "ifany")

dat_long <- dat_long %>%
  mutate(genres_rec = fct_infreq(fct_lump(genres_rec, n = 8))) # Factor keeping 8 most frequent categories and lumping the rest to Other and order the factor by frequency
```
...
genre_count %>%
  ggplot(aes(x = genres_rec, y = n, fill = genres_rec)) +
...
# Length of Movie by Year

```{r, message = FALSE, warning = FALSE}
table(is.na(date_dat$movie_run_time))

date_dat %>%  
  mutate(mins = as.numeric(str_remove_all(movie_run_time, "min"))) %>%
  ggplot(aes(y = yr, x = mins, fill = yr)) +
  geom_density_ridges(alpha = .5) + 
  labs(y = "Release Year", x = "Movie Length (Minutes)") +
  theme_light() + 
  theme(legend.position = "none")
```
