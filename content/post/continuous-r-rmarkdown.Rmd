...
For a review of propensity score analysis with binary treatment, please see Stuart (2010). Below let $e(X)$ denote propensity score, $D$ denote a binary treatment, and $X$ denote all the observed confounders. In the case of binary treatment, propensity scores represent the probability of receiving treatment given the covariates:
...
The data that I use here is from High School and Beyond (HSB) longitudinal study used by Rosenbaum (1986) to analyze the effect of dropping out of high school of later math achievement. For the sake of this example, let's analyze the effect of math efficacy on later math achievement. 
...
Here I am getting the denominators of the IPW, the conditional densities. I have regressed math_efficacy on $X$ and used `dnorm` function to extract the densities. Here, I am not quite sure whether to use the model sigma which divides the errors squared by the degrees of freedom before taking the square root or whether I should just take the standard deviation of the errors. 
...
Please check back on my next post :) 
