...
```{r}
rma_mv_res  <- rma.mv(yi = estimate, 
                      V = v, 
                      mods = ~ 0 + female,
                      random = ~ female | classroom,
                      data = subgroup_fs_res)

tibble(
  rowname = rownames(rma_mv_res$b),
  est = as.vector(rma_mv_res$b),
  SE = rma_mv_res$se,
  ci_lo = rma_mv_res$ci.lb,
  ci_hi = rma_mv_res$ci.ub
) %>%
  kable(digits = 3)
```
